...
const MenuItemCard: React.FC<MenuItemCardProps> = ({ item }) => {
  ...
  return (
    <Card 
      className={`menu-item-card overflow-hidden border border-taj-gold/20 transition-all duration-300 ${isExpanded ? 'shadow-lg' : 'hover:shadow-md'}`}
    ...
    >
      <div className="relative h-48 overflow-hidden">
        ...
        <div className="absolute top-2 right-2 flex gap-1">
          ...
          {item.isSpicy && (
            <span className="bg-red-600 text-white text-xs px-2 py-0.5 rounded-full">Spicy</span>
          )}
          {item.isPopular && (
            <span className="bg-taj-gold text-taj-dark text-xs px-2 py-0.5 rounded-full">Popular</span>
          )}
        </div>
      </div>
            ...
            <div className="flex items-center justify-between mt-4">
              <div className="flex items-center">
                <Button 
                  variant="outline"
                  size="icon"
                  className="h-8 w-8 border-taj-burgundy text-taj-burgundy"
                  onClick={(e) => {
                    e.stopPropagation();
              ...
              <Button
                className={`bg-taj-burgundy hover:bg-taj-burgundy/80 text-taj-cream ${
